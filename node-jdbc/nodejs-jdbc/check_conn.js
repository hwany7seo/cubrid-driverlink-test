...
(async () => {
    try {
        ...
        console.log("conn methods:", Object.keys(conn));
        console.log("conn prototype methods:", Object.getOwnPropertyNames(Object.getPrototypeOf(conn)));
        
        if (conn.setAutoCommit) {
            console.log("conn.setAutoCommit exists");
            await conn.setAutoCommit(false);
            console.log("setAutoCommit(false) called");
            
            const ac = await conn.getAutoCommit();
            console.log("AutoCommit status:", ac);
        } else {
            console.log("conn.setAutoCommit missing!");
        }
