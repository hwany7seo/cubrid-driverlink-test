...
int main() {
    ...
    ret = SQLPrepare(hStmt, (SQLCHAR*)dropTableSQL, SQL_NTS);
    ...
    ret = SQLPrepare(hStmt, (SQLCHAR*)createTableSQL, SQL_NTS);
    ret = SQLExecute(hStmt);
    ...
    SQLFetch(hStmt);
    SQLCloseCursor(hStmt);  // 여기에 커서 정리 추가
    std::cout << "Data count after insert: " << count << std::endl;

    start = std::chrono::high_resolution_clock::now();
    const char* selectSQL = "SELECT * FROM test_table WHERE id = ?";
    ret = SQLPrepare(hStmt, (SQLCHAR*)selectSQL, SQL_NTS);
    checkError(ret, hStmt, SQL_HANDLE_STMT, "Preparing Select Statement");

    SQLINTEGER param_id;
    SQLBindParameter(hStmt, 1, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0, 0, &param_id, 0, NULL);

    SQLINTEGER rowCount = 0;
    for (param_id = 1; param_id <= TEST_COUNT; param_id++) {
        ret = SQLExecute(hStmt);
        checkError(ret, hStmt, SQL_HANDLE_STMT, "Executing Select Statement");
        while (SQLFetch(hStmt) == SQL_SUCCESS) {
            SQLBindCol(hStmt, 1, SQL_C_LONG, &id, 0, NULL);
            SQLBindCol(hStmt, 2, SQL_C_CHAR, name, 256, NULL);
            rowCount++;
        }
