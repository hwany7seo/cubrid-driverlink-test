...
def test_bit_varying(cursor: pyodbc.Cursor):
    _test_vartype(cursor, 'bit varying')
...
def _test_vartype(cursor, datatype):
    ...
    for length in [None, 0, 100, 1000, 4000]:
        ...
        encoding = (datatype in ('blob', 'bit varying')) and 'utf8' or None
...
def test_emoticons_as_parameter(cursor: pyodbc.Cursor):
    ...
    cursor.execute("CREATE TABLE t1(s varchar(100)) charset utf8")
