...
def test_enum_01_insert(cubrid_db_cursor, enum_01_table):
    ...
    cur.execute(f"insert into {enum_01_table} "
...
def test_enum_01_select_cast(cubrid_db_cursor, enum_01_table):
    ...
    rows = cur.fetchall()
    assert [(int(a), int(b)) for a, b in rows] == [(1, 1), (2, 2), (3, 3)]
...
def test_enum_02_insert(cubrid_db_cursor, enum_02_table):
    ...
    cur.execute(f"insert into {enum_02_table} values(1,1,1)")
...
def test_enum_03_insert(cubrid_db_cursor, enum_03_table):
    ...
    cur.execute(f"insert into {enum_03_table} values(1,1,1),"
...
def test_enum_03_update(cubrid_db_cursor, enum_03_table):
    ...
    cur.execute(f"update {enum_03_table} set answers=1")
    assert cur.rowcount in (-1, 3)
...
def test_enum_04_update_1(cubrid_db_cursor, enum_04_table):
    ...
    cur.execute(f"update {enum_04_table} set e1=cast(e2 as int) where e2 < 3")
    assert cur.rowcount in (-1, 4)

    cur.execute(f"select * from {enum_04_table}")
    rows = cur.fetchall()
    assert [tuple(r) for r in rows] == [('a', 'Yes'), ('b', 'No'), ('a', 'Cancel'),
                    ('a', 'Yes'), ('b', 'No'), ('b', 'Cancel')]
...
def test_enum_04_update_2(cubrid_db_cursor, enum_04_table):
    ...
    cur.execute(f"update {enum_04_table} set e2=e1 + 1")
    assert cur.rowcount in (-1, 6)

    cur.execute(f"select * from {enum_04_table}")
    rows = cur.fetchall()
    assert [tuple(r) for r in rows] == [('a', 'No'), ('a', 'No'), ('a', 'No'),
...
def test_enum_04_update_3(cubrid_db_cursor, enum_04_table):
    ...
    cur.execute(f"update {enum_04_table} set e1='b', e2='No'")
    assert cur.rowcount in (-1, 6)

    cur.execute(f"select * from {enum_04_table}")
    rows = cur.fetchall()
    assert [tuple(r) for r in rows] == [('b', 'No'), ('b', 'No'), ('b', 'No'),
                    ('b', 'No'), ('b', 'No'), ('b', 'No')]
