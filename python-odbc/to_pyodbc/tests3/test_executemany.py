...
def test_select(cubrid_db_cursor, exc_many_table):
    ...
    cur.execute(f"INSERT INTO {exc_many_table} (name, category) "
    ...
    cur.executemany(f"select * from {exc_many_table} where name like ?",
        [('sn%',), ('fr%',), ('rac%',)])
    rows = cur.fetchall()
    assert [tuple(r) for r in rows] == [('racoon', 'mammal')]
...
def test_update(cubrid_db_cursor, exc_many_table):
    ...
    rows = cur.fetchall()
    assert [tuple(r) for r in rows] == [('snake', 'update1'), ('frog', 'update2'),
                    ('tuna', 'fish'), ('racoon', 'mammal')]
...
def test_delete(cubrid_db_cursor, exc_many_table):
    ...
    rows = cur.fetchall()
    assert [tuple(r) for r in rows] == [('tuna', 'fish'), ('racoon', 'mammal'),
                    ('name1', 'category1'), ('name2', 'category2')]
...
def test_primary_delete(cubrid_db_cursor, exc_primary_tables):
    ...
    for pat in ('001%', '002%', '003%'):
        with pytest.raises(pyodbc.Error) as ei:
            cur.execute(f"delete from {ptb} where id like ?", (pat,))
