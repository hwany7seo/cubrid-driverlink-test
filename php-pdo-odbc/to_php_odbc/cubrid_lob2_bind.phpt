...
@odbc_exec($conn, 'DROP TABLE IF EXISTS test_blob');
odbc_exec($conn, 'CREATE TABLE test_blob (id INT, contents BLOB)');

// The default type that cubrid_lob2_new will create is BLOB.

$req = odbc_prepare($conn, 'INSERT INTO test_blob VALUES (?, ?)');
