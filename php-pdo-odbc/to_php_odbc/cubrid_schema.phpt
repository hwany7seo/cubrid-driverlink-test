...
if (!$conn = odbc_connect("Driver={CUBRID Driver};server=192.168.2.32;port=33000;uid=dba;pwd=;database=demodb", "", "")) {
...
odbc_exec($conn,"CREATE USER u1;");
odbc_exec($conn,"CREATE TABLE u1.t1(col1 int, col2 varchar(10), col3 double);");
...
odbc_exec($conn,"drop synonym if exists public.s1;");
