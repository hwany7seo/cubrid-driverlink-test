...
$req1 = odbc_exec($conn, "SELECT * FROM timeout2_tb");
...
if(FALSE == $timeout3){
   printf("[003]Expect false,[%d] [%d] [%s]\n",cubrid_error_code_facility(), odbc_error($conn), odbc_errormsg($conn));
}else{
   var_dump($timeout3);
}
...
Warning: cubrid_set_query_timeout() expects parameter 2 to be int, string given in %s on line %d
