...
foreach ($db_params1 as $param_name => $param_value) {
...
foreach ($db_params2 as $param_name => $param_value){
   printf("%-30s,%s\n",$param_name,$param_value);
}
printf("\n");
...
foreach ($db_params3 as $param_name => $param_value){
   printf("%-30s,%s\n",$param_name,$param_value);
}
printf("\n");
...
Warning: Use of undefined constant PARAM_AUTO_COMMIT - assumed 'PARAM_AUTO_COMMIT' (this will throw an Error in a future version of PHP) in %s on line %d

Warning: cubrid_set_db_parameter() expects parameter 2 to be int, string given in %s on line %d
[004]Expect [0] []
