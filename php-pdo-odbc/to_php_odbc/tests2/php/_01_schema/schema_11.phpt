...
odbc_exec($conn,"drop class if exists tt2");
odbc_exec($conn,"drop class if exists tt3");
odbc_exec($conn,"create class hi ( a int , b string )");
odbc_exec($conn,"create class tt1( a int, b string )");
odbc_exec($conn,"create class tt2( a int, b string )");
odbc_exec($conn,"create class tt3( a int, b string )");
odbc_exec($conn,"create trigger tt1_insert after insert on tt1 execute insert into hi(a, b) values( obj.a ,to_char(obj.a))");
odbc_exec($conn,"create trigger tt2_delete before delete on tt2 execute delete from hi");
odbc_exec($conn,"create trigger tt3_alter before update on tt3 execute  update hi set a=a-100 ") ;
odbc_exec($conn,"insert into tt1(a,b) values(1, 'test');");
//odbc_exec($conn,"");
...
Warning: cubrid_schema(): supplied resource is not a valid CUBRID Connect resource in %s on line %d
