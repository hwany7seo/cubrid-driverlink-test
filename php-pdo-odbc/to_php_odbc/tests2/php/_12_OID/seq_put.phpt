--TEST--
cubrid_seq_put (ODBC: UPDATE 로 LIST 요소 치환과 동일 효과)
--SKIPIF--
<?php
...
odbc_exec($conn, 'DROP TABLE IF EXISTS seq_put_tb');
if (!odbc_exec($conn, 'CREATE TABLE seq_put_tb (a int AUTO_INCREMENT, b set(int), c list(int), d char(30)) DONT_REUSE_OID')) {
	printf("[002] CREATE failed [%s] %s\n", odbc_error($conn), odbc_errormsg($conn));
	exit(1);
}
if (!odbc_exec($conn, "INSERT INTO seq_put_tb(a, b, c, d) VALUES (1, {1,2,3}, {11, 22, 33, 333}, 'a')")) {
	printf("[003] INSERT 1 failed [%s] %s\n", odbc_error($conn), odbc_errormsg($conn));
	exit(1);
}
if (!odbc_exec($conn, "INSERT INTO seq_put_tb(a, b, c, d) VALUES (2, {4,5,7}, {44, 55, 66, 666}, 'b')")) {
	printf("[004] INSERT 2 failed [%s] %s\n", odbc_error($conn), odbc_errormsg($conn));
	exit(1);
}

$req = odbc_exec($conn, 'SELECT c FROM seq_put_tb WHERE a = 1');
...
if (!odbc_exec($conn, 'UPDATE seq_put_tb SET c = {111, 22, 33, 333} WHERE a = 1')) {
