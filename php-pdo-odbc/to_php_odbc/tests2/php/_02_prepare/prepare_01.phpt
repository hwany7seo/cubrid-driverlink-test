...
if (false !== ($tmp = @odbc_prepare($conn, "INSERT INTO prepare_tb(c1) VALUES(:%#@)"))) {
    printf("Expecting boolefalse, got %s\n", gettype($tmp), $tmp);
}

if (false !== ($tmp = @odbc_prepare($conn, "INSERT INTO prepare_tb(c1) VALUES(:1adb)"))) {
    printf("Expecting boolefalse, got %s\n", gettype($tmp), $tmp);
}

if (false !== ($tmp = @odbc_prepare($conn, "INSERT INTO prepare_tb(c1) VALUES(':_a-b'')"))) {
    printf("%d--Expecting boolefalse, got %s\n",__LINE__, gettype($tmp), $tmp);
}

if (false !== ($tmp = @odbc_prepare($conn, "INSERT INTO prepare_tb(c1, c5, c6, c7) VALUES('bind time test', ':_aa', ':b3', ?)"))) {
    printf("%d--Expecting boolefalse, got %s\n",__LINE__, gettype($tmp), $tmp);
}

$req = odbc_prepare($conn, "INSERT INTO prepare_tb(c1, c5, c6, c7) VALUES('bind time test', ?, ?, ?)");
...
//date time type
$req = odbc_prepare($conn, "INSERT INTO prepare_tb(c1, c5,c6,c7) VALUES('time and date',?,?,?);");
