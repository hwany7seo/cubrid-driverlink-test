...
EOD;
odbc_exec($conn,$sql);
odbc_exec($conn,"insert into prepare_tb values('string1','char1',1,11.11),('string2','char2',2,222.22222)");

printf("#####correct next_result#####\n");
$sql="insert into prepare_tb values('string1','char1',1,11.11);select * from prepare_tb;";
$res=odbc_exec($conn,$sql, CUBRID_EXEC_QUERY_ALL);
if (false === $res) {
...
if (false === ($tmp=cubrid_next_result($res2))) {
   ...
   if(is_null($row = odbc_fetch_array($res2))){
      printf("[002] [%d] %s\n", odbc_error($conn), odbc_errormsg($conn));
   }else{
...
if (false === ($tmp=cubrid_next_result($res4))) {
   ...
   if(is_null($row = odbc_fetch_array($res4))){
      printf("[004] [%d] %s\n", odbc_error($conn), odbc_errormsg($conn));
   }else{
