...
odbc_exec($conn, 'CREATE TABLE test_lob2 (id INT AUTO_INCREMENT, images BLOB, contents CLOB)');

prepare_table_for_test($conn);

$req = odbc_exec($conn, 'select * from test_lob2');

while ($row = odbc_fetch_row($req, CUBRID_LOB)) {
...
$row = odbc_fetch_row($req, CUBRID_LOB);
...
function prepare_table_for_test($conn) {
    ...
    $req = odbc_prepare($conn, 'INSERT INTO test_lob2(images, contents) VALUES (?, ?)');
