...
$req = odbc_exec($conn, 'SELECT c FROM php_cubrid_test WHERE a = 1');
if (!$req) {
	printf("[004] SELECT failed [%s] %s\n", odbc_error($conn), odbc_errormsg($conn));
	exit(1);
}
...
if ($attr === null) {
	printf("[006] Unexpected LIST column form: %s\n", gettype($raw));
	exit(1);
}
var_dump($attr);

if (!odbc_exec($conn, 'UPDATE php_cubrid_test SET c = {11, 22, 33} WHERE a = 1')) {
