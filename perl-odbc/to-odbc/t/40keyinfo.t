...
if (@$key_info >= 2) {
    # Strip embedded NULs / WCHAR-as-bytes from CUBRID ODBC catalog columns
    like(odbc_unqual_name($key_info->[0]->{TABLE_NAME}), qr/$table/i, "Table name check 1");
    like(odbc_strip_nul($key_info->[0]->{COLUMN_NAME}), qr/^a$/i, "Column a");
    is($key_info->[0]->{KEY_SEQ}, 1);
    
    # PK Name might be auto-generated differently
    # is($key_info->[0]->{PK_NAME}, "pk_test_cubrid_a_b");

    like(odbc_unqual_name($key_info->[1]->{TABLE_NAME}), qr/$table/i, "Table name check 2");
    like(odbc_strip_nul($key_info->[1]->{COLUMN_NAME}), qr/^b$/i, "Column b");
...
if ($sth) {
    ...
    if (@$info) {
        like(odbc_unqual_name($info->[0]->{PKTABLE_NAME}), qr/parent/i);
        like(odbc_strip_nul($info->[0]->{PKCOLUMN_NAME}), qr/^id$/i);
        like(odbc_unqual_name($info->[0]->{FKTABLE_NAME}), qr/child/i);
        like(odbc_strip_nul($info->[0]->{FKCOLUMN_NAME}), qr/^parent_id$/i);
    } else {
