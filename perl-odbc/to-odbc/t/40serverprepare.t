...
# Precision of float/double might vary. 
# 3.0 might come back as 3 or 3.00000 etc.
# is_deeply check might be strict.
# But let's try.
my $rows = $sth->fetchall_arrayref({});
# DOUBLE column often returns fixed-point string (e.g. 3.0000...) via ODBC
cmp_ok(0 + ($rows->[0]->{num} // 0), '==', 3, "Num is 3 (numeric)");
...
ok($dbh->do(q{CREATE TABLE `t2` (`i` int,`si` smallint,`ti` smallint,`bi` bigint)}), "creating test table");
# Changed tinyint to smallint as CUBRID might not have tinyint or ODBC maps it differently? 
# CUBRID has SHORT (smallint). TINYINT? CUBRID usually uses SHORT.
# Let's use SMALLINT for tinyint column to be safe or keep tinyint if supported.
# CUBRID supports TINYINT since recent versions? No, CUBRID uses SHORT.
# Original test used tinyint. CUBRID perl driver probably mapped it.
...
# Compare results
my $res = $dbh->selectall_arrayref('SELECT * FROM t2');
# Deeply check
# ODBC might return strings or numbers.
is($res->[0][0], 101);
is($res->[0][1], 102);
is($res->[0][2], 103);
is($res->[0][3], 104);
